...
** Last update Fri Apr  1 19:01:32 2011 john mille
*/
...
int		go_somewhere(t_vars *v, char **path_tab, char *pwd, char *home)
{
  char		*path;
  char		*tmp;
  int		to_find;
  ...
  path_tab = make_pwd_tab(path_tab, pwd, home);
...
int		my_cd(t_vars *v)
{
  ...
  if (pwd == NULL)
    ...
    exe = move_from_root(v, pwd, home);
  else if (v->argc == 2 && my_strcmp(v->argv[1], "-"))
    {
      pwd_tab = add_path_to_pwd(v);
      exe = go_somewhere(v, pwd_tab, pwd, home);
    }
